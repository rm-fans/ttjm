...
if(opt.queue){jQuery.dequeue(this,opt.queue);}};return opt;};jQuery.easing={linear:function(p){return p;},swing:function(p){return 0.5- Math.cos(p*Math.PI)/ 2;
}};jQuery.timers=[];jQuery.fx=Tween.prototype.init;jQuery.fx.tick=function(){var timer,timers=jQuery.timers,i=0;fxNow=jQuery.now();for(;i<timers.length;i++){timer=timers[i];if(!timer()&&timers[i]===timer){timers.splice(i--,1);}}
			  ...
			  modalBG.unbind('click.modalEvent');
				...
				if(!locked) {
					...
					if(options.animation == "fadeAndPop") {
						modal.css({'top': 0, 'opacity' : 0, 'display' : 'block'});
...
//							"top": $(document).scrollTop()+topMeasure + 'px',
					...
					if(options.animation == "none") {
						modal.css({'display' : 'block', 'top': '0px'});
			  ...
			  if(!locked) {
					...
					if(options.animation == "fade") {
						...
						modal.animate({
							...
							modal.css({'opacity' : 1, 'display' : 'none', 'top' : '0px'});
							unlockModal();
						});					
					}  	
					if(options.animation == "none") {
						modal.css({'display' : 'none', 'top' : '0px'});
    ...
    $.fn.bdrSteps = function ( options ) {
        ...
        var methods = {
            resize_steps: function () {
                ...
                $links.each( function ( index ) {
                    ...
                    if ( array.indexOf( k ) !== -1 ) {
                        $( this ).prop( 'href', '?step=' + k ).css( {color: '#fff'} );
                    }
                } );
            ...
            select: function ( index ) {
                ...
                $fill.stop( true, false ).animate( {width: fill_width}, 1000, function () {
...
//                $('#nav').css({opacity: '0'});
            ...
            var rq = $('#rq'), windowWidth = $(window).width();
            if (windowWidth < 641) {
                rq.html('Quote');
            } else {
...
/*
        ...
        resize: function () {
            var windowHeight = window.innerHeight;
                ...
                elmnts.css({'height': Math.floor(windowHeight) + "px"});
    ...
    service: {
        init: function () {
            ...
            var work = $('#vids').find('li'), content = $('.title, .text, .bd-button, .big-bd-button');
            ...
            $('.show-video').click(function(e){
                ...
                var target = $(this).attr('href'),
                    ...
                    $revealModal = $(target),
                    video_wrap = $(".video");
                if (target == "#video1") { source = video1; }
                if (target == "#video2") { source = video2; }
                if (target == "#video3") { source = video3; }
                ...
                $(target).reveal({ dismissmodalclass: 'close-button' });
                video_wrap.fitVids();
                var w = $(target).find('iframe').width(),
                    h = $(target).find('iframe').height();
                video_wrap.css({'height': h+'px', 'margin-top': -(h/2)+'px'});
            });
...
//            $('#service, .service-item').height(Math.floor(windowHeight));
    ...
    portfolio: {
        init: function () {
            var work = $('#works .work'), workimg = $('#works .work img');
            /*work.hover(function () {
                    work.find('.work-overlay').addClass('fade');
                    $(this).find('.work-overlay').removeClass('fade');
            }, function () {
    ...
    work: {
        init: function () {
            $('.scroll-down').smoothScroll({offset: -55});
        ...
        resize: function () {
            ...
            */
            var windowHeight = window.innerHeight, windowWidth = $(window).width();
...
//                        }
    ...
    contact: {
        init: function () {
            ...
            $('#show-contact-form').smoothScroll({offset: -70});
